    ...
    setPosts((prev) =>
      prev.map((p) =>
        p.id === id
          ? {
              ...p,
              likes: liked.has(id) ? p.likes - 1 : p.likes + 1,
            }
          : p
      )
    );
    ...
    <PageShell maxWidth="lg" showBrand={true}>
      <div className="mb-10">
        ...
        <p className="mt-2 max-w-xl text-ink-muted">
          See updates, stories, and encouragement from families and advocates
          in your circle.
        </p>
      </div>
            ...
            <div className="mb-3 flex items-center gap-3">
              <div
                className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-xs font-bold ${
                  AVATAR_COLORS[p.avatar] ?? "bg-lavender text-purple-deep"
                }`}
              >
                {p.avatar}
              </div>
            ...
            <div className="mt-3 flex items-center gap-4 border-t border-lavender-deep/20 pt-3">
              <button
                ...
                className={`flex items-center gap-1.5 text-xs font-medium transition ${
                  liked.has(p.id)
                    ? "text-purple"
                    : "text-ink-muted hover:text-purple"
                }`}
              >
      ...
      <p className="mt-10 text-center text-sm text-ink-muted">
        <Link href="/dashboard" className="font-medium text-purple hover:text-purple-deep">
          ← Back to Dashboard
        </Link>
      </p>
    </PageShell>
